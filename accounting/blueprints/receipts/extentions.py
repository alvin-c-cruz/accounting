...
class WriteData:
    ...
    def reform_data(self, data):
        ...
        cash = AccountType.query.filter_by(account_type="Cash and Cash Equivalents").first()
        receivable = AccountType.query.filter_by(account_type="Trade Receivable").first()
        preferred_accounts = [account.account_title for account in Accounts.query.filter(
                                        Accounts.account_type_id.in_([cash.id, receivable.id])
                                        ).order_by(
        ...
        revenues = AccountType.query.filter_by(account_type="Revenues").first()
        last_accounts = [account.account_title for account in Accounts.query.filter(
                                        Accounts.account_type_id == revenues.id
                                        ).order_by(
                                                Accounts.account_number.desc()
                                                ).all()
                                        if account.account_title in df_data.columns
                         ]

        other_accounts = Accounts.query.filter(~Accounts.account_title.in_(preferred_accounts + last_accounts)
                                               ).order_by(Accounts.account_number).all()

        other_accounts = [account.account_title for account in other_accounts
                          if account.account_title in df_data.columns]

        self.accounts = preferred_accounts + other_accounts + last_accounts
